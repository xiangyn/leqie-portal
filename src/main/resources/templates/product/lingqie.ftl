	...
	<div class="product">
		<div class="container clearfix">
			...
			<form action="${ctx}/app/order/buy.jhtml" method="post">
				<input type="hidden" name="zhonglei" value="零切"/>
				...
				<div class="form-content">
					...
					<div class="row">
						<div class="col eighth text-right">
							<label class="form-label required">宽度</label>
						</div>
						<div class="col half">
							<div class="form-item">
								<input class="number" type="text" name="kuang" placeholder="请输入宽度" required="required" />
							</div>
						</div>
						<div class="col quarter">
					...
					<div class="row">
						<div class="col eighth text-right">
							<label class="form-label required">厚度</label>
						</div>
						<div class="col half">
							<div class="form-item">
								<input class="number" type="text" name="hou" placeholder="请输入厚度" required="required" />
							</div>
						</div>
						<div class="col quarter">
					...
					<div class="row">
						...
						<div class="col half">
							<input type="hidden" name="type" value="快速" />
							<ul class="order-menus clearfix" data-name="type">
								<li class="half menu-item text-center active" data-value="快速">快速(预计一个工作日到)</li>
								<li class="half menu-item text-center" data-value="优切">优切(预计两个工作日到)</li>
							</ul>
						</div>
					</div>
				</div>
...
$(function() {
	...
	$(".form-content input[name='kuang']").on("change", function() {
		var kuang = $(this).val();
		if($.isNumeric(kuang)) {
			$.ajax({
				url: "${ctx}/app/product/getHoudu.jo",
				type: 'post',
				data: {
					kuandu: kuang,
	...
	$(".form-content input[name='hou']").on("change", function() {
		var hou = $(this).val();
		if($.isNumeric(hou)) {
			$.ajax({
				url: "${ctx}/app/product/getKuandu.jo",
				type: 'post',
				data: {
					houdu: hou,
					xinghaoId: $("input[name='erjimulu']").val()
	...
	$(".product-order input.number").on("change", function() {
		...
		var hou = $("input[name='hou']").val();
		if($.isNumeric(amount) && $.isNumeric(chang) && $.isNumeric(kuang) && $.isNumeric(hou)) {
			$.ajax({
